<template>
    <view class="content">
        ...
        <div ref="logInfo" style="position: fixed; bottom: 0;z-index: 10;font-size: 12px;color: #000;min-height: 100px;width: 100%; padding-bottom: 20px; max-height: 600px; overflow-y: auto;" @click="isLog=!isLog">
            <template v-if="isLog">
                <div style="padding-bottom: 10px;color: #000;">日志信息: <p v-html="logInfo.join('')"></p></div>
                <div style="color: #000;">ErrorCode: {{errorCode.join(",")}}</div>
                <div style="color: #000;">收音调用次数: {{timeNum3}} 次</div>
                <div style="color: #000;">计数器：{{voiceingNum}}次 </div>
                <div style="color: #000;">正在收音：{{isVoiceing}} </div>
            </template>
        </div>
...
export default {
    ...
    watch: {
        ...
        isVoiceModal(value) {
            ...
            // } else {
            //     window.appInfo["_2"]["txtToVoice"]("嗨，我是首都机场,您的语音小助手。您可以点击按钮开始查航班。", false);
            // }
        },
        // 如果语言播报完后
        isVoiceDone(value){
            if(value == true && window.appInfo["_2"]["voiceModal"] == true) {
    ...
    methods: {
                    ...
                    if (res.data.code == "200") {
                        ...
                        if (!!res.data.data.source) {
                            if(res.data.data.source.toLocaleLowerCase() == "clarify") {
                                ...
                                clearInterval(window.appInfo["_2"]["intervalTime"]);
                                window.appInfo["_2"]["stopVoice"]();
                                window.appInfo["_2"]["txtToVoice"](res.data.data.suggestAnswer, true);
                            }
                        }
                        ...
                        if (actions.length > 0) {
                            ...
                            // 
                            if (actions.indexOf("showRichText") != -1) {
                                self.setSuggestAnswer({
                                    type: 2,
                                    suggestAnswer: suggestAnswer
                                });
                        ...
                        self.setIsFright(false);
                        window.appInfo["_2"]["txtToVoice"](res.data.msg, true);
                    }
                },
                fail(error) {
                    console.log(error);
        ...
        txtToVoice() {
            return function (txt, bool) {
                ...
                window.appInfo["_2"]["voiceModal"] = bool;
                if (!!window.jsBridge) {
                    window.jsBridge.hxpApi(
                        11,
                        JSON.stringify({
        ...
        setLog(){
            ...
            return function (logInfo) {
                ...
                if(logInfo.indexOf("subErrorCode")!=-1) {
                    ...
                    // 长时间没对话
                    if(logInfo.indexOf("subErrorCode:3101")!=-1) {
                        ...
                        if( logInfo.indexOf("subErrorCode:8001")!=-1&&
                ...
                // onAsrFinish
                // onAsrExit
                self.isVoiceing = (logInfo.indexOf("640")!=-1);
    ...
    created() {
        ...
        window.appInfo["_2"]["voiceModal"] = false;
        ...
        // window.appInfo["_2"]["setSuggestAnswerInfo"]({
        //     type: 2,
        //     suggestAnswer: "语音组手语音组手语音组手"
        // });
        // window.appInfo["_2"]["setSuggestAnswerInfo"]({
        //     type: 1,
        //     suggestAnswer: "测试测试测试测试"
        // });
